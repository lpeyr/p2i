...
class ArduinoManager:
    ...
    def __init__(self, port, arduino_data_handler=None, baudrate=115200, timeout=0.1):
        self.port = port
    ...
    def open(self):
        ...
        self._listening_thread = threading.Thread(
            target=ArduinoManager._listening_method, args=(self,)
        )
    ...
    def trouver_port_arduino():
        ...
        for p in ports:
            ...
            if my_arduino_port is None and (
                not (p.product is None)
                and "Arduino" in p.product
                or not (p.description is None)
                and "Arduino" in p.description
            ):
